...
/**
 * 功能：这是一个viewGroup容器，实现上下两个frameLayout拖动切换
...
public class DragLayout extends LinearLayout {
    ...
    protected void onFinishInflate() {
        ...
        // 跟findviewbyId一样，初始化上下两个view8360
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (frameView1.getTop() == 0) {
            // 只在初始化的时候调用
            // 一些参数作为全局变量保存起来
            frameView1.layout(left, 0, right, bottom - top);
            frameView2.layout(left, 0, right, bottom - top);

            viewHeight = frameView1.getMeasuredHeight();
            frameView2.offsetTopAndBottom(viewHeight);
        } else {
            // 如果已被初始化，这次onLayout只需要将之前的状态存入即可
            frameView1.layout(left, frameView1.getTop(), right, frameView1.getBottom());
            frameView2.layout(left, frameView2.getTop(), right, frameView2.getBottom());
        }
    }
