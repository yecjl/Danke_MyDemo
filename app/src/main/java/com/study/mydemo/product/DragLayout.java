...
public class DragLayout extends ViewGroup {
    ...
    private static final int VEL_THRESHOLD = 5000; // 滑动速度的阈值，超过这个绝对值认为是上下
    private static final int DISTANCE_THRESHOLD_UP = 50; // 单位是像素，当上滑动速度不够时，通过这个阈值来判定是应该粘到顶部还是底部
    private static final int DISTANCE_THRESHOLD_DOWN = 150; // 单位是像素，当下滑动速度不够时，通过这个阈值来判定是应该粘到顶部还是底部
    ...
    private void animTopOrBottom(View releasedChild, float yvel) {
        ...
        if (releasedChild == frameView1) {
            // 拖动第一个view松手
            // 向上的速度足够大，就滑动到顶端
			if (yvel < -VEL_THRESHOLD || (downTop1 == 0 && frameView1.getTop() < -DISTANCE_THRESHOLD_UP)) {
                // 向上滑动的距离超过某个阈值，就滑动到顶端 (滑动到title底部的位置)
                finalTop = -viewHeight + titleHeight;
            ...
            // 因为frameView1要在title的底部
            finalTop = titleHeight;
            // 拖动第二个view松手
            if (yvel > VEL_THRESHOLD || (downTop1 == -viewHeight + titleHeight && releasedChild.getTop() > DISTANCE_THRESHOLD_DOWN)) {
                // 设置高度，否则保持原地不动
                finalTop = viewHeight;
            }
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (frameView1.getBottom() > titleHeight && frameView1.getTop() < 0) {
