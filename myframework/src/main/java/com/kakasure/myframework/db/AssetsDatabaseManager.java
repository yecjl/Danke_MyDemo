package com.kakasure.myframework.db;
...
//    private static String tag = "AssetsDatabase"; // for LogCat
    ...
    public SQLiteDatabase getDatabase(String dbfile) {
        if(databases.get(dbfile) != null){
            MyLogUtils.i(String.format("Return a database copy of %s", dbfile));
            return (SQLiteDatabase) databases.get(dbfile);
        }
        if(context==null)
            return null;

        MyLogUtils.i(String.format("Create database %s", dbfile));
        ...
        if(!flag || !file.exists()){
            ...
            if(!file.exists() && !file.mkdirs()){
                MyLogUtils.i("Create \""+spath+"\" fail!");
                return null;
            }
            if(!copyAssetsToFilesystem(dbfile, sfile)){
                MyLogUtils.i(String.format("Copy %s to %s fail!", dbfile, sfile));
                return null;
            }
    ...
    private boolean copyAssetsToFilesystem(String assetsSrc, String des){
        MyLogUtils.i("Copy "+assetsSrc+" to "+des);
